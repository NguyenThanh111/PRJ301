        ...
        <%@page import="Models.UserDTO" %>
            <% UserDTO currentUser=(UserDTO) session.getAttribute("user"); String role=(String)
                ...
                <head>
                    ...
                    <title>Auth Logs — Network Manager</title>
                            ...
                            <div class="fv-page-wrap" id="fvPageWrap">
                                <div class="fv-page-header">
                                    ...
                                    <div class="stat-chip" style="background:rgba(16,23,42,0.8);border:1px solid var(--border);border-radius:8px;padding:8px 16px;font-size:0.8rem;color:var(--text-muted);display:flex;align-items:center;gap:8px;">
                                        <i class="bi bi-shield-check" style="color:var(--neon-blue)"></i>
                                        Total: <strong style="color:var(--text-primary);font-size:1rem;" id="fvPageTotal">0</strong> logs
                                    </div>
                                </div>
                            </div>                                        <% if (isAdmin) { %>
                                            <div class="page-section active" id="page-authlogs">

                                                <div class="page-manage-header">
                                                    <div class="page-header-left">
                                                        <div class="page-header-icon"><i class="bi bi-shield-check"></i></div>
                                                        <div>
                                                            <div class="page-header-title">Authentication Logs</div>
                                                        ...
                                                        <table>
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th><i class="bi bi-clock"></i> Login Time</th>
                                                                    <th><i class="bi bi-fingerprint"></i> User ID</th>
                                                                    <th class="text-center">Actions</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody id="authLogTableBody">
                                                                <c:choose>
                                                                    <c:when test="${not empty authLogs}">
                                                                        <c:forEach var="log" items="${authLogs}">
                                                                            <tr data-logid="${log.logId}" data-username="${log.userName}" data-status="${log.loginStatus}" data-success="${log.success}" data-ip="${log.ipAddress}" data-logintime="<fmt:formatDate value="${log.loginTime}" pattern="yyyy-MM-dd HH:mm:ss" />" data-userid="${log.userId}">
                                                                                <td><span class="id-badge">#${log.logId}</span></td>
                                                                                <td class="username-cell"><c:out value="${log.userName}" /></td>
                                                                                <td>
                                                                                    <span class="status-badge ${log.success ? 'status-active' : 'status-inactive'}">
                                                                                        <span class="status-dot ${log.success ? 'status-dot-green' : 'status-dot-red'}"></span>
                                                                                        <c:out value="${log.loginStatus}" />
                                                                                    </span>
                                                                                </td>
                                                                                <td><span style="font-family:monospace;color:var(--neon-cyan);font-size:12px;"><c:out value="${log.ipAddress}" /></span></td>
                                                                                <td style="color:var(--text-muted);font-size:12px;"><fmt:formatDate value="${log.loginTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
                                                                                <td><span class="id-badge" style="color:var(--neon-purple);border-color:rgba(139,92,246,0.3);background:rgba(139,92,246,0.08);">${log.userId != null ? log.userId : 'N/A'}</span></td>
                                                                                <td>
                                                                    ...
                                                                    <c:otherwise>
                                                                        <tr><td colspan="7"><div class="empty-state"><i class="bi bi-shield-check"></i><p>No authentication logs found.</p></div></td></tr>
                                                                    </c:otherwise>
                                                                </c:choose>
                                                            </tbody>
                                                        </table>
    ...
    <div class="detail-overlay" id="detailOverlay" onclick="if(event.target===this)closeDetail()">
        <div class="detail-panel">
            ...
            <div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;">
                <div class="detail-avatar" id="detailAvatar">L</div>
                <div>
            ...
            <div class="detail-grid" id="detailFields">
                ...
                <div><div class="detail-section-label">Status</div><div class="detail-value">-</div></div>
                <div><div class="detail-section-label">IP Address</div><div class="detail-value">-</div></div>
                <div><div class="detail-section-label">Login Time</div><div class="detail-value">-</div></div>
                <div><div class="detail-section-label">User ID</div><div class="detail-value">-</div></div>
            </div>
        </div>
    </div>
                    ...
                    <script>
                        ...
                        function computeStats() {
                            ...
                            allRows.forEach(function(r) {
                                if (r.getAttribute('data-success') === 'true') success++; else failed++;
                            });
                            document.getElementById('statsBar').innerHTML =
                                '<div class="stat-chip"><i class="bi bi-shield-check" style="color:#60a5fa"></i> Total: <strong>' + total + '</strong></div>' +
                                '<div class="stat-chip"><span class="status-dot status-dot-green" style="width:7px;height:7px;"></span> Success: <strong>' + success + '</strong></div>' +
                                '<div class="stat-chip"><span class="status-dot status-dot-red" style="width:7px;height:7px;"></span> Failed: <strong>' + failed + '</strong></div>';
                        }
                        ...
                        function openDetail(btn) {
                            ...
                            var loginTime = row.getAttribute('data-logintime');
                            var userId = row.getAttribute('data-userid');

                            document.getElementById('detailOverlay').classList.add('show');
                            document.getElementById('detailId').textContent = '#' + logId;
                            document.getElementById('detailUsername').textContent = '@' + username;
                            document.getElementById('detailName').textContent = username;

                            var detailFields = document.getElementById('detailFields');
                            detailFields.innerHTML =
                                '<div><div class="detail-section-label">Login Status</div><div class="detail-value"><span class="status-badge ' + (success ? 'status-active' : 'status-inactive') + '"><span class="status-dot ' + (success ? 'status-dot-green' : 'status-dot-red') + '"></span> ' + status + '</span></div></div>' +
                                '<div><div class="detail-section-label">IP Address</div><div class="detail-value" style="font-family:monospace;color:#22d3ee;">' + ip + '</div></div>' +
                                '<div><div class="detail-section-label">Login Time</div><div class="detail-value">' + loginTime + '</div></div>' +
                                '<div><div class="detail-section-label">User ID</div><div class="detail-value" style="font-family:monospace;color:#60a5fa;">' + (userId || 'N/A') + '</div></div>';

                            var avatar = document.getElementById('detailAvatar');
                            avatar.textContent = (username || '?').charAt(0).toUpperCase();
                            avatar.style.background = success ? 'linear-gradient(135deg,#34d399,#059669)' : 'linear-gradient(135deg,#f87171,#dc2626)';
                        }
